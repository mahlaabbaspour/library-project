...
import { useCreateStaff } from '@/hooks/admin/staff/useStaff'
...
export default function CreateStaffModal({ open, onClose }: Props) {
  const [dateTime, setDateTime] = useState<Dayjs | null>(dayjs().calendar('jalali'))
  ...
  const onSubmit = async (values: FormValue) => {
    try {
      ...
      if (dateTime) {
        formData.append('employment_date', dateTime.calendar('gregory').format('YYYY-MM-DD'))
      }
      ...
      await createStaff({ payload: formData })
      reset()
      setAvatarFile(null)
  ...
  return (
    <Card>
      ...
      >
        ...
        <DialogTitle>ایجاد کارمند</DialogTitle>
          ...
          <Grid container spacing={6}>
            ...
            <Grid item md={4}>
              <CustomDatePicker
                label='تاریخ استخدام'
                value={dateTime}
                onChange={(date: Dayjs | null) => setDateTime(date)}
              />
            </Grid>
